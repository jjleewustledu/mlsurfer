classdef SurferData 
	...
	methods
        ...
        function m = cohortThicknessIndex(this)  
            m = this.cohortIndex('Path',      this.subjectsDir, ...
                                 'Parameter', this.parameter, ...
                                 'Territory', this.territory);          
        end        
        function m = cohortOefratio(this)
            oefCortex = this.cohort('Path',      this.subjectsDir, ...
                                    'Parameter', 'oefnq_default_161616fwhh', ...
                                    'Territory', this.territory);
            oefCereb = this.cohortCerebOefnq('Path', this.subjectsDir);           
            for b = 1:length(oefCortex)
                m{b} = oefCortex{b}/oefCereb{b};
            end
        end
