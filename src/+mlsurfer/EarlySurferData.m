classdef EarlySurferData 
	%% EARLYSURFERDATA contains and manages cortical thickness and surface data.
	...
	methods (Static)
        ...
        function sd       = processAllJames(subjids)
            ...
            for s = 1:length(subjids) 
                sd = EarlySurferData.process4James(jamfid, subjids{s});
            end            
            fclose(jamfid);
        end % static processAllJames
        function sd       = process4James(jamfid, subjid)
            import mlsurfer.*;
            sd   = EarlySurferData(pwd, subjid);
        ...
        function thickavg = processMcaThickAvg(subjsdir, subjid)            
            import mlsurfer.*;
            sd = EarlySurferData(subjsdir, subjid);
        ...
        function sd       = processHemisphere(jamfid, subjid)
            import mlsurfer.*;
            disp('KLUDGE:  set hemispheres in EarlySurferData.HEMIS');
            sd   = EarlySurferData(pwd, subjid);
            cd(fullfile(subjid, 'stats', ''));
        ...
        function names    = collectData(contents)
            ...
            for c = 1:length(contents)
                if (strfind(contents{c}, EarlySurferData.DATA_START))
            ...
            try
                names = regexpi(contents2, EarlySurferData.EXPRESSION, 'names');
    ...
    methods
        ...
        function fns = atlasFqfns(this, hemis)
            ...
            for a = 1:length(this.ATLASES) %#ok<*FORFLG>
                ...
                assert(2 == exist(fns{a}, 'file'), 'EarlySurferData.atlasFqfns could not find %s', fns{a});
            end
        end % atlasFqfns
	...
	methods (Access = 'protected')
        ...
        function this = populateMaps(this, hemis, content)
            ...
            import mlsurfer.*;
            names = EarlySurferData.collectData(content); 
            for n = 1:length(names)
                fields = fieldnames(names{n});
                for f = 1:length(fields)                       
        ...
        function ref  = mapReference(~, hemis, field)
 		...
 		function this = EarlySurferData(pwdtop, sid)
            
 			%% EARLYSURFERDATA (ctor) 
 			%  Usage:   processed_data = EarlySurferData(specifiers)
            import mlsurfer.*;
